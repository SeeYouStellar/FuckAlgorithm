...
int post[30], in[30], N;
node* create(int inl, int inr, int postl, int postr){     //由后序遍历和中序遍历，建立二叉树
    if(postl > postr) return NULL;
    node* root = new node();
    root->data = post[postr];  //后序遍历的每棵二叉树的根节点都位于后序遍历序列的最后一个
    int i, numleft;
    for(i=inl;i<=inr;i++)  //找到中序遍历序列中的根节点，根据根节点的性质可以将根节点的左子树和右子树找出来，即该中序遍历序列中根节点下标的左边和右边
        if(in[i] == root->data)
            break;
    numleft = i - inl;   //关键，每次找到左子树节点个数
